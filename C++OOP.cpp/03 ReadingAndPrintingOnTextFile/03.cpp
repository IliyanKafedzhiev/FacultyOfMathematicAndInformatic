//orders.txt ID_data_служител дд-мм-гггг
//ordersDetails.txt Id_поръчка продукт единична цена, количество;
//Да създадем трети файл ID дата, служител, обща стойност на поръчката
...
struct order{
    ...
    bool read(istream&); // keyboard, file
    ostream& print(ostream&);
};

bool order::read(istream& in){ // Четена на поръчка
    total=0;
...
struct orderDetail{
    ...
    bool read(istream&); // аналогично като за предишната структура
    ostream& print(ostream&);
